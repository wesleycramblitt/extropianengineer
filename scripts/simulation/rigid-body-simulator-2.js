...
export class RigidBodySimulator {
    constructor(rigidBodies, settings) {
        this.staticBodies = rigidBodies.filter(x => x.type == "static");
        this.dynamicBodies = rigidBodies.filter(x => x.type == "dynamic");
        this.rigidBodyState = {
    ...
    step(dt) {
      for (var dynamicBody of this.dynamicBodies) {
